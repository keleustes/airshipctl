...
import (
	...
	pluginenv "opendev.org/airship/airshipctl/pkg/helm/environment"
)
...
const PluginSettingsID = "helm"

// NewKubectlCommand creates a new command for working with kubectl
func NewHelmCommand(rootSettings *environment.AirshipCTLSettings) *cobra.Command {
	args := os.Args[1:]
	...
	flags := pluginRootCmd.PersistentFlags()

	settings.AddFlags(flags)

	out := pluginRootCmd.OutOrStdout()

	pluginRootCmd.AddCommand(
		// chart commands
		newCreateCmd(out),
