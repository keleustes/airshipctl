...
import (
	...
	"k8s.io/klog"
	"opendev.org/airship/airshipctl/pkg/client/restmapper"
...
type Client interface {
	...
	WaitForClusterV1alpha3Ready() error
...
func New(kubeconfig string) (*client, error) { //nolint
	...
	c, err := NewFromDefaultSearchPath(f, restmapper.NewConfigOverrides())
	if err != nil {
		return nil, err
	}
...
func (c *client) EnsureNamespace(namespaceName string) error {
	clientset, err := restmapper.NewCoreClientSetForDefaultSearchPath(c.kubeconfigFile, restmapper.NewConfigOverrides())
	if err != nil {
...
func NewFromDefaultSearchPath(kubeconfigFile string, overrides tcmd.ConfigOverrides) (*client, error) { //nolint
	...
	if err := util.PollImmediate(retryAcquireClient, timeoutAcquireClient, func() (_ bool, err error) {
		c, err = restmapper.NewControllerRuntimeClient(kubeconfigFile, overrides)
		if err != nil {
...
func (c *client) WaitForClusterV1alpha3Ready() error {
